...
must produce **byte-identical** results on the parity fixtures. The player-facing explanation of the same rules is
[`RULES.md`](RULES.md). When SPEC.md and this document disagree about the rules, this document wins (see SPEC.md §0).
...
The ruleset merges five inputs: the lead's baseline and four proposals (*fun*, *competitive*, *engineering*,
*physics*), scored by three judges (player, competitive, engineer).

- **Game-design core: *competitive*.** It had the highest mean score across the three judges: competitive 7.18,
  fun 6.68, engineering 6.55, physics 5.47, baseline 5.13. It was also the only proposal that all three judges
  scored at 6.75 or higher.
- **Numeric and encoding spine: *engineering*.** All three judges asked for integer-only maths.
- **Grafts** come from *fun* and *physics* wherever at least two judges agreed. Where the judges disagreed, the
  decision and the reason are recorded below.
- **Red-team review.** A later review added D17 and D18 and changed D3, D4, D6, D8, D13 and D14. The reasons are in
  the rows themselves; the resulting edge cases are rows 65–86 and A1–A6 of §11.

| # | Decision | Rationale | Source |
|---|---|---|---|
| D1 | The state is a **public probabilistic mixture of classical boards** ("worlds"). There are no amplitudes and no interference. | Odds are something a player can work out, captures are always definite, and the model is cheap to compute. All three judges rejected signed amplitudes. JS and PHP floats do not cancel reliably (the engineer judge verified `H·H ≠ 0.5`), and partial merges are unreadable for players. | baseline, competitive, engineering; all judges |
| D2 | **Integer weights with a fixed total `T = 2^24`.** A split uses ceil/floor. Rescaling uses the largest-remainder rule. The state holds no floats, the JSON is byte-identical, and parity has no tolerance. | PHP and JS print floats differently (`1.0e-7` vs `1e-7`). A 1e-12 tolerance lets the two engines sample different outcomes at a boundary. Exact rationals grow without bound. | engineering; all three judges |
| D3 | **Kings are always classical.** They never split or merge, and every non-castling king move is in the measured class M (castling is never in M, §4.5, §4.6). | No king-hiding and no surviving by a defensive coin flip. King danger is cheap to compute exactly. | competitive, fun; all judges |
| D4 | **There is no check rule.** Every move that is otherwise legal stays legal whatever it does to your king. You win by capturing the king. Instead there is a **king-danger ring** that always shows its percentage and a **confirmation dialog** that is on by default. These are UI aids, not rules. The only related rule is a *termination* rule: a side whose every move leaves its king certainly capturable has lost at once (D17). | Two of three judges (competitive, engineer) rejected a check layer. It adds a second legality notion, and it brings "reckless move" losses. The player judge's concern about instant blunder losses is handled by the confirmation, which asks from 10% risk upwards (§8) so that cheap low-odds king shots do not go unnoticed. The ring turns red at 100% ("certain danger"), which is exactly the player judge's semantic check: "the opponent has a move that captures the king with probability 1". Chess players still get check-like moments without a second legality notion. | competitive, engineering; player-judge concern adopted as UI; red-team review |
| D5 | **Per-side quantum budget: `B(c) ≤ 8`.** B(c) counts the distinct arrangements of side c's own pieces. `worlds ≤ 64` follows from it. A split that would exceed the budget is illegal. A non-measured move that would exceed it is resolved by a roll (*budget fallback*); it is never forbidden. | The opponent can never use up your capacity (proof in §7.2). A shared global cap allows capacity-denial griefing (all judges). Fun's "3 ghost slots" was not taken over: in a mixture model the thing to bound is correlation, not pieces. From fun we keep a visible pip meter, and we explain the number as "three 50/50 splits". | competitive; competitive and engineer judges; UI from fun |
| D6 | **Readability limits.** A split may not leave the piece on more than `MAX_LOCATIONS = 4` squares. Split targets must be empty in **every** world, including the splitting piece itself. There is **no** minimum branch weight: a branching world of weight 1 sends its single unit to child 1 (§4.7). | This meets the player judge's main objection to arbitrary weights. A split can no longer build a 5-square cloud, and it can no longer land on its own part, which was the main source of 3:1 and 5:3 odds. A minimum branch weight (engineering's `MIN_BRANCH_WEIGHT = 2`) was dropped after review: every classical piece stands in every world, so one weight-1 world, which the *opponent* can create, would forbid all of your splits. That is exactly the capacity denial D5 rules out. | engineering (cap); fun and physics (empty targets); red-team review (no branch-weight rule). This **changes** engineering and competitive, which allowed a split onto the piece's own part. |
| D7 | **A split is two half-weight standard moves.** A child whose lane is blocked stays on the source square. The split also needs at least one world where both lanes are clear. | The same blocking rule applies everywhere. No split can quietly move the whole piece to one target. | competitive plus engineering S4; competitive and engineer judges |
| D8 | **A merge is a converging move and may capture** (*converging capture*). If the result is uncertain it is rolled. If X reaches the target in **every** world (so X stands only on the two merged squares, or already on the target, and no lane can be blocked), the capture is certain and no number is drawn. | "Preparation beats dice": good play turns uncertainty into certainty. It also fixes fun's edge case #22 (a ghost queen attacking the king with both halves). | competitive; all three judges |
| D9 | **Measure move.** Spend your turn to measure one of your own superposed pieces. It has at most 8 outcomes. | It gives the player a choice at the budget limit, frees stuck ghosts, and teaches measurement as a deliberate act. You can measure only your own pieces. | physics ("Observe" in the player judge); all three judges |
| D10 | **When a roll happens.** A move is rolled only if it is a pawn move, a king move, a merge or standard move that *lands* on a square another piece might hold, a Measure, or a budget fallback. A slide past a possibly-occupied square links the pieces without a roll. A number is drawn only when there are at least 2 outcomes. The outcome key is `miss` / `move` / `capture`. | Fun's mnemonic is "land = roll, pass = link". A chance node has at most 3 children. Forced outcomes use no randomness (physics). | baseline, engineering, physics, fun |
| D11 | **Pawns stay classical**, and promotion and en passant are always definite. `ep` is set only when an en-passant capture is actually available. | The chess skeleton stays solid, and the repetition key carries no `ep` noise. | all proposals; engineering (`ep`) |
| D12 | **Castling rights follow the state.** A right is lost when, after a move, the king or that rook is not certainly on its home square. A missed or blocked attempt keeps the right. | It is as deterministic and cheap as the attempt-based rule. Attempt-based loss punishes moves that never happened (player judge). The competitive judge preferred attempt-based; this is overruled and documented here. | fun, physics; player judge |
| D13 | **Termination rules.** Threefold repetition is automatic, using FNV-1a-64 over the exact state. The 50-move rule counts only events that actually happened. Bare kings is a draw, no legal move is a draw, and there is a technical cap of 1200 plies. There is **no** K+minor draw. The bare-kings, repetition and 50-move draws **yield to a pending certain king capture** (D18). | Split and merge shuffling can no longer stall a game. Without check, K+minor versus K may be winnable (competitive and engineer judges). | engineering, competitive |
| D14 | **Randomness.** Online, a fresh CSPRNG value is drawn when the move is applied, and no seed is stored. Each move record carries a SHA-256 hash chain, which detects later changes to moves a client has **already loaded**; it cannot detect a roll that someone with database access changes before the opponent's client first loads the move (§9.4). Local games draw a fresh CSPRNG value the first time a rolled move is played in a given position and remember it for that game (*roll memo*), so undo cannot re-roll the same move in the same position, and no future value exists that could be read in advance. The trainer uses forced outcomes. | A commit-reveal seed kept on the server lets an admin see every future roll, and the admin is often a player (all judges). A local HMAC seed was replaced after review: the seed sits in browser storage, so a player could compute every future roll, and a different promotion letter gave a fresh roll after an undo. Two-party verifiable dice need a secret on the opponent's device; they are deferred (Appendix D). | competitive; all judges; red-team review |
| D15 | **Determinism discipline.** Every state carries `v`. Key order is fixed. Codes are canonical. The move list has a total integer order. The engine provides a lenient parser, `whyIllegal` reason codes, `isLegal` / `hasAnyLegalMove`, and `applyMove({u} \| {outcome})`. | These make bit-identical twin engines possible and give the LLM useful feedback when it retries. | engineering; all judges |
| D16 | **Honesty contract** for the trainer, the coach and LLM prompts (Appendix E). | The game models superposition, measurement and entanglement as correlation. It does not model interference or non-locality. | physics; all judges |
| D17 | **Trapped king ends the game** (`king_trapped`, E1b). After a move, if every legal move of the side to move leaves the game running with its own king certainly capturable (`kingTrapped`, §6), the mover wins at once. | Without it every forced win needs one pointless extra ply from the loser, and the loser controls when it happens: in a correspondence game the winner could wait days for a timeout. It is a termination rule, not a legality rule, so D4 still holds. It reuses `generateMoves`, `getOutcomes` and `kingDanger`, which are already parity-tested. | red-team review |
| D18 | **Draws yield to a certain king capture.** E2 (bare kings), E3 (repetition) and E4 (50 moves) are checked only if the side to move has no certain king capture. | Otherwise taking the last defended piece with the king next to the enemy king, or blundering on the 100th half-move, would turn a lost position into a draw. Chess avoids this because moving into check is illegal and mate takes precedence over the move-count rules. | red-team review |
...
- `GameService` draws `u = random_int(0, 2^24 − 1)` **while applying the move**, and only if the move is rolled.
...
- SPEC.md's default `rng` (`random_int(0, 2^53−1)/2^53`) is equivalent through §5.2.
...
### 9.4 Game record integrity (normative for GameService and the online client)
...
**App-level edge cases** (Appendix D, SPEC.md; not engine rules):
...
- **Fixtures** (`tests/fixtures/engine/*.json`). Each step records:
  ...
  (`successProbability`, `probability`) are excluded from byte comparison. This replaces SPEC.md's 1e-12 rule.
...
`feedback = whyIllegal` text, then falls back to the built-in engine (SPEC §5.2).
...
## Appendix D. App-level rules to adopt in SPEC.md / GAME-DESIGN.md

These rules are not part of the engine. They are recommended by the judges and the red-team review.

- **Randomness and record.** Use §9.2 to §9.4. Add a `chain` string(64) column to `qchess_moves`, and store the measurement record from §5.5.
- **Time.** Use correspondence deadlines of 1, 3 or 7 days per move, with a default of 3. Send reminders at 50% and 90% of the deadline.
  ...
  - Live Fischer clocks are post-v1, after `notify_push` has been evaluated.
- **Rated play.**
  ...
  - Use Elo with K = 40 for the first 10 rated games (shown as provisional "?"), then K = 20. Glicko-2 with an RD column is the preferred follow-up.
  - Results against the engine, the LLM, or in pass & play never affect rating.
- **Dice trust.** The server applies every online roll, so v1 dice are only as trustworthy as the server (§9.2, §9.4).
  - In rated games, a player who is a member of the instance's `admin` group SHOULD be shown with an "admin" badge that links to the dice FAQ entry (RULES.md).
  - *Post-v1, non-normative:* two-party "verified dice" for rated games. At join, each client keeps a secret `x` and publishes `anchor = SHA256^600(x)`. A rolled move is stored as `roll_pending` with `commit = SHA256(r)` for fresh server randomness `r`. The opponent's client, on its next fetch, checks the pending move and reveals the next element `e` of its hash chain; the server checks `e` against the previous element, reveals `r`, and applies `u` = the first 24 bits of `SHA256(r ‖ e ‖ chain_{n−1})`; both clients verify. It is deferred because `x` must live on the opponent's device (a second device or cleared storage breaks the game) and the mover sees the result only when the opponent next opens the game.
- **Fair play.** In a player's own active rated games, the coach, hints, eval bar, analysis and position export are disabled.
  ...
  - This only raises the bar. The built-in engine runs in the browser and cannot be switched off by the server, so rated fair play rests on trust, and RULES.md says so.
- **UI.**
...
## Appendix F. Changes this document requires in SPEC.md

- **§3.1 and §3.2.**
  - `applyMove` gains `u` (JS `opts.u`; a PHP `?int $u` parameter). Precedence is outcome, then u, then rng.
  - The Measurement shape is §5.5. The API may add float `probability` fields.
  - LegalMove has the fields of §4.10.
  - New functions: `isLegal`, `whyIllegal`, `hasAnyLegalMove`, `positionHash`, `budget`, `kingDanger`, `kingTrapped`, `moveRisk`, `links`, `rollDisplay`.
  - Constants: `T`, `BUDGET`, `MAX_LOCATIONS`, and `MAX_WORLDS = 64` (derived). There is no `MIN_BRANCH_WEIGHT`.
  - Engine result reasons add `king_trapped`.
- **§3.3.** Fixtures store `u` and compare bytes. The 1e-12 tolerance is removed. Parser fixtures (§4.12) are added.
- **§5.1.** Add `qchess_moves.chain`. `measurement` holds the §5.5 record. The chain uses `created_at` as Unix seconds (§9.4).
- **§5.2.** `POST /api/ai/move` gets the same rated-game check as `/api/ai/coach` (Appendix D).
- **§5.3.** Rating and time rules as in Appendix D. Elo K = 32 is replaced by 40/20.
- **§5.4.** Prompts use Appendix B, and the retry `feedback` uses `whyIllegal` codes and texts.
- **Local games.** The saved local game stores the roll memo `rolls` and each move's `u` (§9.3). There is no `gameSeed`.
