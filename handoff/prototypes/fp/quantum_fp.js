...
import { rescaleWeights } from '/home/user/Quantum_Chess/src/engine/index.js'
import { applyClassical, generate, HAND, nameOf, OFF, worldKey } from '/home/user/Quantum_Chess/src/variants/core/world.js'
...
/**
 * PROTOTYPE HOOK: the budget of the side to move (variant hook budgetLimit(world), default BUDGET).
 *
 * @param {object} V variant
 * @param {object} state state
 * @return {number}
 */
export function budgetLimit(V, state) {
	return V.budgetLimit ? V.budgetLimit(state.worlds[0].b) : BUDGET
}
...
function moveBranches(V, state, key) {
	...
	if (budgetOf(r.worlds, state.turn) > budgetLimit(V, state)) {
		return toBranches(r.worlds, true)
	}
	return toBranches(r.worlds, false)
}
...
function splitBranches(V, state, mv) {
	...
	if (merged.length > MAX_WORLDS || budgetOf(merged, state.turn) > budgetLimit(V, state)) {
		return null
	}
...
function mergeBranches(V, state, mv) {
	...
	if (enemyMaybe || budgetOf(worlds, state.turn) > budgetLimit(V, state)) {
...
export function stateAfter(V, state, code, branch, all, { light = false } = {}) {
	...
	if (!next.result && !light && !hasLegalMove(V, next) && V.passWhenStuck) {
		// PROTOTYPE HOOK: a side without a legal move sits out; the next side that can move is to move
		let probe = next
		for (let i = 1; i < V.sideCount; i++) {
			const t = nextSide(V, probe.worlds[0].b, probe.turn)
			if (t === next.turn) {
				break
			}
			probe = { ...probe, turn: t }
			if (hasLegalMove(V, probe)) {
				next = { ...probe, skipped: true }
				break
			}
		}
	}
