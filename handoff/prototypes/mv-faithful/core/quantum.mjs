...
import { applyClassical, generate, HAND, nameOf, OFF, worldKey } from '/home/tom/Projects/Quantum_Chess/Quantum_Chess/src/variants/core/world.js'
 ...
 * @return {number}
 */
export function budgetOf(worlds, side, V) {
	const set = new Set()
	for (const { b } of worlds) {
		set.add(projection(b, side, V))
	}
	return set.size
}
 ...
 * @return {number}
 */
export function budget(state, side, V) {
...
function splitBranches(V, state, mv) {
	...
	state.worlds.forEach(({ b, w }, i) => {
		...
		const w2 = w - w1
		worlds.push({ b: m1 ? applyClassical(V, b, m1) : b, w: w1, k: 'move', cap: -1, idle: !m1 })
		if (w2 > 0) {
			worlds.push({ b: m2 ? applyClassical(V, b, m2) : b, w: w2, k: 'move', cap: -1, idle: !m2 })
		}
	})
	if (!branching) {
		return null
	...
	if (merged.length > MAX_WORLDS || budgetOf(merged, state.turn, V) > BUDGET) {
		return null
	}
...
function measureBranches(V, state, mv) {
	...
	return [...g.entries()]
		...
		.map(([sq, list]) => ({
			...
			rolled: true,
			notes: [],
			worlds: idleApply(V, state, { type: 'measure', code: mv.code, id: X, from: mv.from, to: [] }, list.map(({ b, w }) => ({ b, w, k: 'move', cap: -1, idle: true })), false),
			captures: [],
		}))
}
