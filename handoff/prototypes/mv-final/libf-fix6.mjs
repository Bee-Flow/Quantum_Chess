...
import V, * as P from './mvf-fix6.mjs'
import * as Q from '../../tmp/mv-final/core/quantum.mjs'
	...
	for (let y = n - 1; y >= 0; y--) {
		...
		for (let xx = 0; xx < n; xx++) {
			...
			cells.push(view.map((o) => {
				const b = P.base(o.type)
				const ch = o.side === 0 ? b.toUpperCase() : b
...
export function show(state, { history = true } = {}) {
	...
	out.push(`ply ${state.ply} turn ${state.turn} worlds ${state.worlds.length} budget W${Q.budget(state, 0)} B${Q.budget(state, 1)} created ${x.c} present v${sk.present} (T${sk.present >> 1}${sk.present & 1 ? 'b' : 'w'}) submit ${P.canSubmit(x)} result ${JSON.stringify(state.result)}`)
	for (let u = 0; u < P.ROWS; u++) {
		const e = x.tl[u]
		if (!e) {
			continue
		}
		out.push(`  L${P.LAB[u]} [${e[0]}..${e[1]}] parent ${e[2]},${e[3]} ${sk.act(u) ? 'active' : 'inactive'}`)
		const from = history ? Math.max(e[0], e[1] - x.h) : e[1]
		for (let v = from; v <= e[1]; v++) {
			out.push(`    v${v} T${v >> 1}${v & 1 ? 'b' : 'w'}${v === e[1] ? '*' : ' '} ${boardText(state, u, v)}`)
		}
	}
	return out.join('\n')
}
