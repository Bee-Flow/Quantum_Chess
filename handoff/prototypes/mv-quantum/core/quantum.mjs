...
import { rescaleWeights } from '/home/tom/Projects/Quantum_Chess/Quantum_Chess/src/engine/rescale.js'
import { applyClassical, generate, HAND, nameOf, OFF, worldKey } from './world.mjs'
 ...
 * @return {number}
 */
export function quantumAllowed(V, state, action) {
	// PROTOTYPE PATCH (proposed hook allowQuantum): the variant may forbid a split, merge or measurement
	return !V.allowQuantum || V.allowQuantum(state, action)
}
...
function mergeCandidates(V, state, f) {
	...
	for (const other of locs) {
		...
		if (!quantumAllowed(V, state, { type: 'merge', from: [f, other].sort((a, b) => a - b), to: [] })) {
			continue
		}
	...
	for (const { b } of state.worlds) {
		for (let id = 0; id < b.sq.length; id++) {
			...
			if (superposed(state, id)) {
				if (!must) {
					// PROTOTYPE PATCH: list the measurement at the first part the variant allows
					const fm = locs.find((f) => ownPieceAt(state, f) === id
						&& quantumAllowed(V, state, { type: 'measure', from: [f], to: [] }))
					if (fm !== undefined) {
						out.push({ code: '?' + nameOf(V, fm), type: 'measure', from: [fm], to: [] })
					}
				}
 ...
 * @param {boolean} hit whether some world of the same outcome took the action
 * @return {object[]}
 */
...
function splitBranches(V, state, mv) {
	...
	if (!V.types[home.ty[X]].splittable || !quantumAllowed(V, state, { type: 'split', from: [f], to: [t1, t2] })) {
		return null
	}
	const { gens } = table(V, state)
	const entries = []
...
function perWorldMerge(V, state, mv) {
	...
	// PROTOTYPE PATCH (bug fix): read the type where X stands on f1; X may be absent from the first world
	const home = state.worlds.find(({ b }) => b.board[f1] === X).b
	if (!V.types[home.ty[X]].splittable || facesOf(state, X, [f1, f2]).size > 1
		|| !quantumAllowed(V, state, { type: 'merge', from: [f1, f2].sort((a, b) => a - b), to: [t] })) {
		return null
	}
	const { gens } = table(V, state)
	const find = (i, from) => {
...
function measureBranches(V, state, mv) {
	...
	if (X < 0 || !superposed(state, X) || !quantumAllowed(V, state, { type: 'measure', from: [mv.from[0]], to: [] })) {
		return null
	}
...
/**
 * The outcome key (and captures) of one part of a branch that a settling roll split: a miss, move or capture branch
 * takes the key and the captures of its own worlds; a split part whose worlds are all idle is a miss; other keys (a
 * measured square, `gone`) stay.
 *
 * @param {Branch} br the branch that was split
 * @param {object[]} ws the worlds of the part
 * @return {{key?: string, captures?: number[]}}
 */
function partLabel(br, ws) {
	if (br.key === 'miss' || br.key === 'move' || br.key === 'capture') {
		return { key: resultKey(ws), captures: capturesOf(ws) }
	}
	if (br.key === 'split' && ws.every((e) => e.idle)) {
		return { key: 'miss' }
	}
	return {}
}
