...
import { rescaleWeights } from '../../../src/engine/index.js'
import { applyClassical, generate, HAND, nameOf, OFF, worldKey } from '../../../src/variants/core/world.js'
...
export function mergesFrom(V, state, f) {
	...
	for (const other of locs) {
		...
		if (facesOf(state, X, [f, other]).size > 1) {
			continue
		}
...
function mergeBranches(V, state, mv) {
	...
	// proposed for shogi: parts that show different faces (promoted in some worlds only) cannot merge
	if (facesOf(state, X, [f1, f2]).size > 1) {
		return null
	}
	const { gens } = table(V, state)
	const find = (i, from) => {
...
/**
 * The types piece X has in the worlds where it stands on one of the given squares.
 *
 * @param {QState} state state
 * @param {number} X piece id
