const Q = '/home/user/Quantum_Chess/src/variants/core/quantum.js'
...
// worlds: [[placement, rel, hand?[[side,type]]], ...]
...
export function show(V, s) {
  return s.worlds.map(({ b, w }) => {
    const pcs = b.sq.map((q, id) => q >= 0 ? `${b.sd[id]}${b.ty[id]}#${id}@${nameOf(V, q)}` : q === HAND ? `hand${b.sd[id]}:${b.ty[id]}#${id}` : null).filter(Boolean).join(' ')
    const x = { ...b.x }; if (x.castle) x.castle = x.castle.map(c => c.flag ?? c.side).join('')
    return (w / T).toFixed(4) + ' ' + pcs + ' ' + JSON.stringify(x)
  })
}
