...
/** prototype only: the variant whose budget groups apply */
export const CURRENT = { V: null }
 ...
 * @return {string}
 */
function projection(b, side, V = CURRENT.V) {
	const group = V && V.budgetSides ? V.budgetSides(side) : [side]
	const parts = []
	for (let id = 0; id < b.sq.length; id++) {
		if (group.includes(b.sd[id]) && b.sq[id] !== OFF) {
