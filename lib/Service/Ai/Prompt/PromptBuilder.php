...
/**
 * Builds the prompts of the LLM opponent and of the coach.
 *
 * A prompt contains only the position, move codes, the legal moves, the computer player's numbers, the persona, the
 * text the user typed and the language. It never contains user ids, names, e-mail addresses or instance URLs. Text
 * from the user is quoted as «…» and treated as a message, not as instructions.
 */
class PromptBuilder {
	/** The rules in a few lines, for the system prompts. A test checks its key phrases against docs/rules.md. */
...
TXT;
	...
	/**
	 * English explanations of the reason codes of `whyIllegal()` (docs/engine-rules.md §4.11) for the retry line, plus
	 * the client's `not_recommended`.
	 */
	...
	/**
	 * The prompt of the LLM opponent: persona, rules, the position, recent moves, the computer player's candidates and
	...
	/**
	 * The prompt of the coach: its rules of conduct, the position, the computer player's analysis, the context (game,
	...
	/**
	 * The candidate tags of the computer player (for example `capture:50:p`) in English; unknown tags are dropped.
	 *
