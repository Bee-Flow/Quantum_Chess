...
/**
 * The OpenAI chat completions API, which OpenAI, Mistral, OpenRouter, Groq, Gemini, Ollama, LocalAI, LM Studio and
 * many other servers offer.
 *
 * A request parameter that a model rejects with a 400 answer is dropped, remembered, and the request is sent once
 * more without it.
 */
final class OpenAiProvider extends HttpProvider {
	/** Model ids that are not chat models and are left out of the model list. */
	...
	public function chat(array $messages, array $options): ChatResult {
		...
		return ChatResult::done($content);
	}
	...
	private function headers(): array {
		...
		if ($this->config['preset'] === 'openrouter') {
			// never HTTP-Referer: it would reveal the instance's URL
