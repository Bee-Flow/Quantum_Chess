...
/**
 * The computer player's move choice: the search plus each level's personality — softmax temperature, random and
 * top-half picks, split/merge/measure policies, king-shot thresholds, how often it notices danger to its own king,
 * the roll bonus, the level-5 exact solver, and the display delay.
 *
...
import { captureWeight, cleanValue, victimOf } from './searchValues.js'
import { solve } from './solver.js'
import { freshSeed, NO_SLICE, runSync } from './tasks.js'
 ...
 * @return {boolean}
 */
function partAttacked(feat, id) {
	const enemy = id < 16 ? 1 : 0
	for (let s = 0; s < 64; s++) {
		if (feat.occ[s] === id && feat.att[enemy * 64 + s] > 0) {
			return true
		}
	}
	return false
 ...
 * @param {LegalMove[]} moves legal moves
...
function policyPool(state, moves, level, funSplit) {
	...
	let pool = moves.filter((m) => {
		...
		if (m.type === 'merge') {
			return level.merges === true || (level.merges === 'hanging' && partAttacked(feat, m.piece))
		}
		if (m.type === 'measure') {
			return level.measures === true || (level.measures === 'useful' && (full || partAttacks(feat, m.piece)))
		}
		return true
	})
	if (funSplit) {
 ...
 * @param {object} level level
 * @param {() => number} rng noise RNG
 * @param {((code: string) => boolean)|null} [safe] moves the non-random picks may choose (the best always may)
 ...
 * level's node budget and no clock. Result: `{code, E, depth, nodes, candidates: [{code, E}], displayMs}` — `E` is
 * White's expected score after the move.
 *
 ...
 * @return {object}
 */
export function* bestMoveTask(state, options = {}, ctx = NO_SLICE) {
	...
	if (level.kingShotMin !== null) {
		...
		pool = pool.filter((m) => kingShot(feat, m) === 0)
	}
...
export function bestMove(state, options = {}) {
	return runSync(bestMoveTask(state, options))
}
