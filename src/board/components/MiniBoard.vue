...
<!--
  A small, light board for lobby cards, the possibilities panel, the rules page and the trainer path: one SVG, ghosts
  drawn with opacity only (no badges). Changing `state` animates solid pieces to their new squares.
-->
<template>
	<svg
...
import { ghostOpacity, isLightSquare, squareCenter, squareXY } from '../geometry.js'
...
const arrowList = computed(() => props.arrows.map((a) => {
	const from = squareCenter(Array.isArray(a.from) ? a.from[0] : a.from, props.orientation)
	const to = squareCenter(a.to, props.orientation)
