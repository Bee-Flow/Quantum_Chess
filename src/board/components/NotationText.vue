...
<!--
  Display notation: the engine's `moveNotation` with sprite figurines (none for pawns), en dash and ×, the split/merge
  pipe in the quantum colour and the roll result as a small tag ("✓ 50%", "○ Moved 50%", "○ Missed 75%"). The canonical
  text stays available to screen readers and copy.
-->
<template>
	<span class="qc-notation" :class="{ 'qc-notation--missed': result && result.key === 'miss' }" :title="notationText">
		<span class="hidden-visually">{{ notationText }}</span>
