...
/**
 * Integer board sizing: `S = max(36, floor(min(availW, availH, 880) / 8))`, measured on the game view container. The
 * layout follows the container's width rather than the window's, so at 200 % zoom the view falls back to the stacked
 * layout.
 */
...
/**
 * The square size, board size and layout of a game view, kept up to date while the view is mounted.
 *
 * @param {import('vue').Ref<HTMLElement|null>} containerRef game view container
 * @param {object} [options] options
 * @param {number} [options.reserved] extra vertical space in px to keep free
 * @return {{squareSize: import('vue').Ref<number>, boardPx: import('vue').Ref<number>,
 *   layout: import('vue').Ref<string>}}
 */
export function useBoardSize(containerRef, { reserved = 0 } = {}) {
	...
	useResizeMeasure(() => containerRef.value, () => {
