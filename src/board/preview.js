...
/**
 * The move preview, the "odds card": every number comes from `LegalMove.outcomes`, `getOutcomes()` and `moveRisk()`,
 * memoised per (position hash, code). Pure functions; `MovePreview` renders the result.
 */
 ...
 *
 * @param {EngineState} state state (types)
 ...
 *
 * @param {EngineState} state state before
 ...
 *
 * @param {EngineState} state state before
 * @param {LegalMove} move a legal move of `state`
