...
<!--
  The evaluation bar: White's expected score E as the fill, the fog band (min–max of E across the first roll on the
  principal line) and "♚ in N" for a proven certain win. Vertical beside the board, or a thin horizontal bar above it on
  phones. Placed absolutely next to the board by its container.
-->
<template>
	<div
		class="qc-eval"
...
.qc-eval__fog {
	position: absolute;
	inset-inline: 0;
	background: repeating-linear-gradient(45deg, var(--qc-eval-fog) 0 2px, var(--qc-eval-fog-light) 2px 4px);
...
.qc-eval__label {
	...
	&--white {
		bottom: 2px;
		color: var(--qc-eval-white-text);
	}

	&--black {
		top: 2px;
		color: var(--qc-eval-black-text);
	}
