...
/**
 * The coach of a game (SPEC §14.4.7, GAME-DESIGN §5.3): analysis for the eval bar, threat warnings and
 * opportunities, move-quality badges for the player's moves and hints (tiers 1–2). Numbers come from the AI worker
...
/** Analysis time per position (GAME-DESIGN §5.3.1). */
...
export function useCoach({ state, moves, myColor, enabled = ref(true), stateAt = null, level: fixedLevel = null, deps = {} }) {
	...
	/** Analyse the current position (the player's turn only: the engine needs the worker on its own turn). */
	function schedule() {
		clearTimeout(timer)
		ctrl?.abort()
