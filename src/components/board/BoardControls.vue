...
<!--
  The move switcher (GAME-DESIGN §3.5.1: Move / Split / Merge / Measure, keys 1–4, disabled with a reason), the
  preview text of the hovered target (the odds card condensed, for phones) and the pending Cancel / Play buttons
  (GAME-DESIGN §3.5.6). Shares the BoardInput object with QuantumBoard.
-->
<template>
...
const buttons = computed(() => {
	...
	const list = [
		...
		// TRANSLATORS: move type that makes a piece a ghost on two squares (RULES.md glossary)
		{ value: 'split', icon: mdiCallSplit, label: t('quantumchess', 'Split'), key: 2 },
		// TRANSLATORS: move type that joins two parts of a ghost (RULES.md glossary)
		{ value: 'merge', icon: mdiCallMerge, label: t('quantumchess', 'Merge'), key: 3 },
		// TRANSLATORS: move type that settles where a ghost really is (RULES.md glossary)
