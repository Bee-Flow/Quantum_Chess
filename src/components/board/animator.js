...
/**
 * The board's move and roll timeline (SPEC §14.4.2, GAME-DESIGN §3.6.1): travel, suspense with the outcome ring,
 * settle, collapse, then the reveal arrow and the result chip. It only drives reactive state; `QuantumBoard` draws it.
 *
...
/** Base durations in ms (GAME-DESIGN §3.2), multiplied by the speed factor. */
 ...
 * @param {(event: object) => {chip: object|null, reveal: object|null, speech: string}} [hooks.describe] result chip,
...
export function createAnimator({ speed, sound = () => {}, suspense = () => () => {}, describe = () => ({ chip: null, reveal: null, speech: '' }), announce = () => {} }) {
	 ...
	 * @param {object} input.move the LegalMove
	 * @param {'self'|'opponent'} [input.actor] who moved (the opponent's rolls have no travel)
	 * @return {{resolve: (result: {after: object, measurement: object, names?: object, lessonRoll?: boolean}) => Promise<void>, fail: () => Promise<void>}}
	 */
	 ...
	 *
	 * @param {object} event MoveEvent {before, after, move, measurement, actor, names, lessonRoll}
	 * @return {Promise<void>}
	 */
