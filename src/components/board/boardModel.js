...
/**
 * Pure display data of the board (GAME-DESIGN §3.3–§3.4): the pieces to draw (ghost opacity, rings, badges, identity
 * dots, what-if deltas), king rings, last-move squares, square labels for screen readers, the reveal arrow and the
 * outcome ring of a roll. Everything comes from the engine views, memoised per position hash.
 */
 ...
 * The pieces to draw.
 *
 * Each item: `{key, piece, square, type, color, weight, probability, ghost, parts, idColour, chosen, impossible,
 * delta}`. `key` is `<piece id>:<square>`. With `whatIf` (a square holding a ghost part) the probabilities are the
 * conditional ones (ER §8): `chosen` marks that part, `impossible` a part that cannot be there in this case, and
 * `delta` (`{up, percent}`) a square whose chance changed by at least one percentage point.
 *
...
export function pieceItems(state, { whatIf = null, identity = {} } = {}) {
	...
	for (let square = 0; square < 64; square++) {
		...
		out.push({
			...
			ghost: weight < T,
			parts,
			idColour: parts.length > 1 ? (identity[v.piece] ?? null) : null,
...
/**
 * King rings (GAME-DESIGN §3.4.5) for the given colours.
 *
 * @param {object} state engine state
...
/**
 * Squares of the last move (GAME-DESIGN §3.3): filled, or outlined with a dashed line for a Missed ("attempted").
 *
...
/**
 * The accessible label of a square (GAME-DESIGN §9.1): "f3, white knight, 50 percent, also on h3".
 *
 * @param {object} state engine state
...
/**
 * The reveal arrow of a roll (GAME-DESIGN §3.6.1): from the attempted square to where the ghost really was, when the
 * result depends on it; otherwise null.
 *
 * @param {object} before state before the move
 * @param {object} move the LegalMove
