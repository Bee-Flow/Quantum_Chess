...
<!--
  Game over (GAME-DESIGN §3.9): the tipped king (or ½), the headline from the player's point of view, the reason in
  plain words, the score line, the game summary and the actions, with variants per result and quantum confetti on a
  win.
-->
<template>
	<NcDialog
		...
		<template #actions>
			...
			<NcButton v-if="canReview" :variant="primary === 'review' ? 'primary' : 'secondary'" @click="emit('review')">
...
import { reasonCopy } from '../../services/format.js'
...
const primary = computed(() => {
	...
	if (outcome.value === 'loss' && props.canReview) {
		return 'review'
	}
	return 'rematch'
})
...
const lossLine = computed(() => {
	...
	const other = props.myColor === 'w' ? 'b' : 'w'
