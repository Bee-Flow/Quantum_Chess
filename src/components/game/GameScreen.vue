...
<!--
  The game screen shared by every mode (SPEC §14.5, GAME-DESIGN §3.1): opponent card, board, own card and controls,
  the move strip on phones, and the panel (below on phones and tablets, on the right on desktops). It drives the board
  through a GameController and owns the history view, the game-over dialog and the resign confirmation.
  Slots: banners, overlay (over the board), eval (left of the board), chat and coach (panel tabs), actions (panel
  footer, replaces the local Undo/Resign buttons).
-->
<template>
	<div
		ref="root"
		...
		<div class="qc-game__column">
			<div v-if="historyPly !== null || banners.length || $slots.banners" class="qc-game__banners">
				<NcNoteCard v-if="historyPly !== null" type="info" class="qc-game__banner">
					<div class="qc-game__banner-row">
						<span>{{ t('quantumchess', 'Viewing move {n} of {total}', { n: historyPly, total: moves.length }) }}</span>
			...
			<PlayerCard
				...
				:isMe="myColor === top"
				:showDanger="preferences.kingDangerBoth || myColor === top || myColor === null"
				:compact="compact"
				:over="!!result">
				<template #status>
					<NcButton
...
const top = computed(() => (bottom.value === 'w' ? 'b' : 'w'))
