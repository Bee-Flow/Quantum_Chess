...
<!--
  A player card (GAME-DESIGN §3.8): avatar, name and colour swatch, rating and admin chip (online), budget pips,
  captured tray, king danger, turn status and the AI comment bubble.
-->
<template>
	<div
		class="qc-player"
		...
		<div class="qc-player__main">
			<div class="qc-player__line">
				...
				<span v-if="player.adminBadge" class="qc-player__admin" :title="t('quantumchess', 'Administrator of this Nextcloud')">{{ t('quantumchess', 'admin') }}</span>
			...
			<div class="qc-player__line qc-player__line--details">
				...
				<KingDangerChip v-if="showDanger && dangerWeight > 0" :weight="dangerWeight" :opponent="!isMe" />
			</div>
		</div>
...
import PersonaAvatar from '../ai/PersonaAvatar.vue'
import BudgetPips from '../board/BudgetPips.vue'
import KingDangerChip from '../board/KingDangerChip.vue'
import PieceIcon from '../board/PieceIcon.vue'
import { PIECE_VALUES } from '../../ai/levels.js'
import { budget, kingDanger } from '../../engine/index.js'
import { moodToExpression } from '../../personas/index.js'
...
const materialLead = computed(() => {
	const other = props.player.color === 'w' ? 'b' : 'w'
...
.qc-player__disc--ai {
	box-shadow: 0 0 0 2px var(--qc-ai, #7c4dff);
}
