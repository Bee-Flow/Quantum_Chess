...
<!--
  Recent online games on Home (GAME-DESIGN §2.3 item 7): the last five finished or aborted games with the result,
  the reason, the rating change, Review and Rematch.
-->
<template>
		...
		<ul class="qc-recent__list">
			<li v-for="r in rows" :key="r.game.id" class="qc-recent__row">
				...
				<span class="qc-recent__actions">
					<NcButton
						v-if="canReview && r.game.status === 'finished'"
						variant="tertiary"
...
import { otherPlayer, outcomeText } from '../../online/summaryText.js'
