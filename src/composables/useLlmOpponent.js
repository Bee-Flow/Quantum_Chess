...
/**
 * The AI opponent move protocol (GAME-DESIGN §6.4, SPEC §14.4.4): engine candidates in the worker, one request to
 * the AI source (Nextcloud AI tasks are polled), validation with `findMove`, one retry with feedback, and a fallback
 * to the best ✓ candidate (marked `ai-fallback`, ⚙ badge) with a canned persona line. After 3 fallbacks within 5 AI
 * moves the game switches to index mode. *Let the engine move* appears after 20 s; the engine moves by itself after
 * 120 s. Rolls of the chosen move are made by the caller (roll memo).
 */

import { getLanguage } from '@nextcloud/l10n'
import { computed, ref } from 'vue'
import { findMove, whyIllegal } from '../engine/index.js'
import { cannedLine } from '../personas/index.js'
 ...
 *
 * @param {object|null} legal LegalMove
 ...
 * @param {object} options.persona persona of src/personas
 * @param {string} options.source AI source id
...
export function useLlmOpponent({ record, persona, source, model = null, strength = 'balanced' }, deps = {}) {
	...
	/**
	 * Load the engine client lazily (keeps the worker code out of the main chunk).
	 *
	 ...
	 *
	 * @param {number} ply ply of the AI move
	 */
	 ...
	 *
	 * @param {object} state position (the AI is to move)
	 * @param {object} [options] options
	...
	/** Stop waiting for the AI source and let the built-in engine choose. */
	...
	/**
	 * Something to say to the AI, sent with the next request (≤ 200 characters).
	 *
