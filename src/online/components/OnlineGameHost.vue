...
<template>
	...
	<GameScreen
		...
		<template v-if="offerBanner || rematchBanner || rematchLink || c.rematchState.value === 'pending'" #banners>
			<GameBanner v-if="offerBanner" :text="offerBanner" data-test="draw-offer">
			...
			<GameBanner v-if="rematchBanner" :text="rematchBanner" data-test="rematch-offer">
			...
			<GameBanner
				...
				:text="t('quantumchess', 'You asked {name} for a rematch. Waiting for an answer.', { name: c.names.value[other] })"
			...
			<GameBanner v-if="rematchLink" type="success" :text="t('quantumchess', 'The rematch is on.')">
		...
		<template v-if="started && c.participant.value" #coach>
			<div class="qc-online__coach">
				...
				<template v-else>
					...
					<NcButton variant="primary" @click="review">
...
import GameBanner from '../../game/components/GameBanner.vue'
import GameScreen from '../../game/components/GameScreen.vue'
...
const other = computed(() => otherColor(c.myColor.value))
