...
/**
 * The lobby store (SPEC §14.4.6, GAME-DESIGN §7.3, §7.10): a singleton seeded from the `lobby` initial state, so Home
 * and the navigation paint without a request. `start()` polls the cheap summary with its ETag (30 s, 15 s while Home
 * shows the lobby, 120 s in a hidden tab) and loads the full lobby only when the token changed.
 */
 ...
 *
 * @return {object} {lobby, counts, loading, error, refresh(), start(), stop(), poke(), watchFast(), accept(id), decline(id), cancel(id), join(id), rematch(id)}
 */
