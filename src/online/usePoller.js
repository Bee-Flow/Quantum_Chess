...
/**
 * A polling loop (SPEC §14.4.5, GAME-DESIGN §7.10): one request at a time, the next one scheduled after the previous
 * answer with an interval that the caller computes from the context (visible or hidden tab), an immediate poll when
 * the tab becomes visible or the network comes back, exponential backoff on errors (×2, ±20 % jitter, capped at
 * 120 s), `Retry-After`, and a connection state for the banners.
 */
...
/**
 * @param {(options: {signal: AbortSignal}) => Promise<any>} fetchFn one poll; throws ApiError-like errors
 * @param {object} options options
 * @param {(ctx: {hidden: boolean}) => number|null} options.interval next delay in ms, null = wait for pollNow()
 * @param {Function} [options.onWake] called when the tab becomes visible or the network returns, before polling
 * @param {Function} [options.random] jitter source (tests)
