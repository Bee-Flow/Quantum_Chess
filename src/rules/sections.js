...
export const RULES_SECTIONS = [
		...
		blocks: [
			{
				...
				items: [
					() => t(
						'quantumchess',
						'It’s chess. Same board, same pieces, same moves. Castling, en passant and promotion all exist.',
					),
			...
			{
				type: 'board',
				setup: { fen: 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1', prelude: ['g1-f3|h3'] },
				caption: () => t('quantumchess', 'A split knight: 50% on f3, 50% on h3.'),
			},
		],
	},
	{
		...
		blocks: [
			{
				...
				rows: [
					[
						() => t('quantumchess', 'A normal piece'),
						() => t('quantumchess', 'Solid: the piece is on that square for certain (100%).'),
					],
					[
						() => t('quantumchess', 'A faded piece with a percentage badge'),
						() => t('quantumchess', 'One part of a ghost. The piece is on this square with that chance.'),
					],
					[
						() => t('quantumchess', 'A dotted line between faded pieces'),
						() => t('quantumchess', 'These are the parts of one piece.'),
					],
					[
						() => t('quantumchess', 'A thin thread with a chain icon'),
						() => t('quantumchess', 'These pieces are linked: where one is tells you where the other is.'),
					],
					[
						() => t('quantumchess', 'A ring around a king'),
						() => t('quantumchess', 'King danger: the chance the opponent could capture this king with one move right now. Red at 100%.'),
					],
					[
						() => t('quantumchess', 'Pips next to a player’s name'),
						() => t('quantumchess', 'That side’s quantum budget (0–8 used).'),
					],
				],
			},
	...
	},
	{
		...
		blocks: [
			...
			{ type: 'p', text: () => t('quantumchess', 'Split: a knight, bishop, rook or queen moves to two squares at once. Both targets must be squares the piece could normally reach, and both must be empty for certain. Each target gets half of the chance the piece had. A split never captures and never rolls the dice.') },
			{
				...
				play: { code: 'g1-f3|h3' },
				caption: () => t('quantumchess', 'g1-f3|h3: the knight splits.'),
			},
			...
			{
				type: 'board',
				setup: { fen: '7k/p7/8/8/8/8/8/3QK3 w - - 0 1', prelude: ['d1-d4|h5', 'a7-a6'] },
				play: { code: 'd4|h5-h8' },
				caption: () => t(
					'quantumchess',
					'd4|h5-h8: the queen merges onto the king. Captured for certain, no dice.',
				),
			},
			...
			{
				type: 'board',
				setup: { fen: '4k3/p7/8/8/8/8/8/4K1N1 w - - 0 1', prelude: ['g1-f3|h3', 'a7-a6'] },
				play: { code: '?f3' },
				caption: () => t('quantumchess', 'Measuring the knight: it turns out to be on f3 or on h3.'),
			},
		],
	},
	{
		...
		blocks: [
			{
				...
				rows: [
					...
					[
						() => t('quantumchess', 'Quantum'),
						() => t(
							'quantumchess',
							'No dice, but afterwards your piece is (or stays) in several places, or it becomes linked.',
						),
					],
					[
						() => t('quantumchess', 'Roll'),
						() => t(
							'quantumchess',
							'The game rolls. The chance of every result is shown before you commit.',
						),
					],
					[
						...
						() => t(
							'quantumchess',
							'Normally quantum, but your budget is full, so it is settled with a roll.',
						),
					],
				],
			},
			{ type: 'p', text: () => t('quantumchess', 'The game rolls only when a move could turn out in more than one way and it lands on a square where a piece is or might be, or it is a pawn or king move, a Measure, or your budget is full. Nothing else ever rolls, and nothing collapses on its own.') },
			{
				...
				rows: [
					[
						() => t('quantumchess', 'Captured'),
						() => t('quantumchess', 'Your piece moved and took the piece on the target square.'),
					],
					[
						() => t('quantumchess', 'Moved'),
						() => t('quantumchess', 'Your piece moved. The target square turned out to be empty.'),
					],
			...
			{
				type: 'board',
				setup: { fen: '4k1n1/p7/8/8/8/8/8/2B1K3 w - - 0 1', prelude: ['e1-d1', 'g8-f6|h6'] },
				play: { code: 'c1-h6' },
				caption: () => t(
					'quantumchess',
					'A solid bishop lands on a ghost: 50% Captured, 50% Moved. Afterwards the board catches up.',
				),
			},
			...
			{
				type: 'board',
				setup: { fen: '3rk3/p7/8/8/8/8/8/3QK3 w - - 0 1', prelude: ['d1-d3|a4', 'a7-a6'] },
				play: { code: 'd3-d8' },
				caption: () => t('quantumchess', 'The queen’s d3 part attacks the rook: Captured or Missed.'),
			},
			...
			{
				type: 'board',
				setup: { fen: '4k3/8/1n6/8/8/8/7P/R3K3 w - - 0 1', prelude: ['h2-h3', 'b6-a4|c4'] },
				play: { code: 'a1-a8' },
				caption: () => t(
					'quantumchess',
					'a1-a8 passes a4: the rook is now 50% a1, 50% a8, linked to the knight.',
				),
			},
		],
	},
		...
		blocks: [
			{
				...
				items: [
					() => t(
						'quantumchess',
						'Pawns are always solid. They never split or merge, and every pawn move is settled at once.',
					),
					...
					() => t(
						'quantumchess',
						'A double step needs both squares free. If the way is blocked, the pawn stays where it was.',
					),
			...
			{
				type: 'board',
				setup: { fen: '4k3/8/8/8/6n1/8/4P3/4K3 w - - 0 1', prelude: ['e1-d1', 'g4-e3|h6'] },
				play: { code: 'e2-e4' },
				caption: () => t('quantumchess', 'Pawns make great probes: e2-e4 finds out where the knight is.'),
			},
		],
	},
	{
		...
		blocks: [
			{
				...
				items: [
					() => t(
						'quantumchess',
						'The king is always solid. A king step onto a square where a piece might be is a roll.',
					),
					() => t(
						'quantumchess',
						'There is no check. You may move your king into danger; if you do, your opponent may take it.',
					),
			...
			{
				type: 'board',
				setup: { fen: '7k/p7/8/8/8/8/8/3QK3 w - - 0 1', prelude: ['d1-d4|h5'] },
				caption: () => t('quantumchess', 'Both parts of the queen attack the black king: king danger 100%.'),
			},
		],
	},
	{
		...
		blocks: [
			{ type: 'p', text: () => t('quantumchess', 'You win when you capture the enemy king, when the enemy king cannot escape after your move, or when your opponent resigns or, online, runs out of time.') },
			{
				...
				items: [
					...
					() => t(
						'quantumchess',
						'Draw: 50 moves by each side without a capture or a pawn move actually happening.',
					),
					() => t(
						'quantumchess',
						'Draw: the player to move has no legal move, or the game reaches 600 moves by each side.',
					),
				],
			},
		...
		blocks: [
			{ type: 'p', text: () => t('quantumchess', 'The quantum budget counts the different ways your own pieces could be standing: one 50/50 knight gives 2, add a 50/50 bishop and you have 4, add a 50/50 rook and you have 8, which is full.') },
			{
				...
				setup: {
					...
					prelude: ['g1-f3|h3', 'a7-a6', 'b1-a3|c3', 'b7-b6'],
				},
				caption: () => t('quantumchess', 'Two white ghosts: White’s budget is 4/8.'),
			},
			{
				type: 'list',
				items: [
					...
					() => t(
						'quantumchess',
						'Your opponent can never use up your budget; their moves can only lower it.',
					),
		...
		blocks: [
			{
				...
				items: [
					...
					() => t(
						'quantumchess',
						'The built-in engine and AI opponents do not know future rolls. They see the same odds you do.',
					),
				],
			},
		],
		...
		blocks: [
			{
				...
				items: [
					...
					() => t(
						'quantumchess',
						'Games against the computer, an AI model or pass & play never change your rating.',
					),
				],
			},
		],
	...
	},
	{
		...
		blocks: [
			...
			{
				type: 'faq',
				q: () => t('quantumchess', 'Can I re-roll by undoing?'),
				a: () => t(
					'quantumchess',
					'No. In local games the same move in the same position always gets the same result.',
				),
			},
			{
				type: 'faq',
				q: () => t('quantumchess', 'The percentages say 67% and 33%. Why not halves?'),
				a: () => t('quantumchess', 'After a roll, the remaining possibilities share the chance in proportion.'),
			},
		],
	},
	{
		...
		blocks: [
			{
				...
				rows: [
					[
						() => t('quantumchess', 'Budget'),
						() => t(
							'quantumchess',
							'How many different ways your own pieces might be standing. At most 8 per side.',
						),
					],
					[
						() => t('quantumchess', 'Captured / Moved / Missed'),
						() => t('quantumchess', 'The three results of a rolled move.'),
					],
					[
						() => t('quantumchess', 'Converging capture'),
						() => t('quantumchess', 'A merge onto an enemy piece.'),
					],
					[
						() => t('quantumchess', 'Ghost'),
						() => t('quantumchess', 'A piece that is on several squares at once, each with a chance.'),
					],
					[
						() => t('quantumchess', 'Link'),
						() => t('quantumchess', 'Two or more pieces whose positions depend on each other.'),
					],
					[
						() => t('quantumchess', 'Measure'),
						() => t('quantumchess', 'A move that settles where one of your own ghosts really is.'),
					],
					[
						() => t('quantumchess', 'Possibility'),
						() => t('quantumchess', 'One complete chessboard that could be the real one, with its chance.'),
					],
					[
						() => t('quantumchess', 'Roll'),
						() => t('quantumchess', 'The random decision when a move could turn out in several ways.'),
					],
					[
						() => t('quantumchess', 'Solid'),
						() => t('quantumchess', '100% on one square. Kings and pawns are always solid.'),
					],
					[
						() => t('quantumchess', 'What-if view'),
						() => t('quantumchess', 'The board as it would be if the part you tapped is the real one.'),
					],
				],
			},
		],
