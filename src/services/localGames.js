...
/**
 * Local games (computer, AI opponent, pass & play) in `localStorage` with the roll memo (SPEC §14.7, ENGINE-RULES
 * §9.3). Index `quantumchess.localGames.v1` = `[{id, mode, updatedAt, result, players}]`, one record per game under
 * `quantumchess.localGame.v1.<id>`. At most 50 records; the oldest finished ones are pruned first.
 */

import { applyMove, initialState, rollIdentity, validateState } from '../engine/index.js'
import { localGameId } from './ids.js'
 ...
 * @param {object} [options.options] {coach, autoFlip, tabletop}
 * @return {object} the record
 */
...
/**
 * The roll memo (ENGINE-RULES §9.3): the same move in the same position always gets the same `u`. A new identity
 * draws a value, stores it in `record.rolls` and **saves the record before** the caller applies the move.
 *
 * @param {object} record local game record
...
/**
 * Apply one recorded move.
 *
 * @param {object} state state before
 * @param {object} entry {code, u, key}
