...
/**
 * The board playback of the lesson and puzzle screens: the shown position, the last-move highlight, the animation
 * handshake with the board, and the replies of the computer player.
 */
...
/**
 * Create the playback of one board.
 *
 * @param {import('vue').Ref<object|null>} boardRef StandaloneBoard component ref
 * @return {object} {state, lastMove, set(state), play(code, {outcome, actor, lessonRoll}),
 *   engineReply(level, outcome?)}
 */
export function usePlayback(boardRef) {
	 ...
	 *
	 * @param {EngineState|null} s position
	 */
	function set(s) {
		state.value = s
		lastMove.value = null
	...
	async function play(code, { outcome = null, actor = 'self', lessonRoll = false } = {}) {
		...
		try {
			await boardRef.value?.play({
				...
				actor,
				names: { mover: names[mover], opponent: names[otherColor(mover)] },
	...
	/**
	 * Let the computer player reply.
	 *
	 * @param {number} level computer player level 1–5
