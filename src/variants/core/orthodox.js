 ...
 * `x.castle` (the castling rights: `[{ flag, side, king, rook, kingTo, rookTo }]`).
 */
...
export function castlingRights(V, w, opts = {}) {
	...
	for (const side of [0, 1]) {
		...
		const ks = w.sq[king]
		const [kf, kr] = topo.coords[ks]
		const rooks = w.sq
			.map((s, id) => ({ s, id }))
			.filter(({ s, id }) => s >= 0 && w.sd[id] === side && w.ty[id] === 'r' && topo.coords[s][1] === kr)
		const long = rooks
		...
		if (short) {
			out.push({
				...
				king: ks,
				rook: short.s,
				kingTo: topo.at([kingToShort, kr]),
				rookTo: topo.at([kingToShort - 1, kr]),
			})
		}
		if (long) {
			out.push({
				...
				king: ks,
				rook: long.s,
				kingTo: topo.at([kingToLong, kr]),
				rookTo: topo.at([kingToLong + 1, kr]),
			})
		}
	}
	return out
...
/**
 * The squares strictly between two squares on one rank (empty when they are equal or adjacent).
 *
 ...
 * @return {number[]}
 */
function between(topo, a, b) {
	const [fa, r] = topo.coords[a]
	const fb = topo.coords[b][0]
	const out = []
	for (let f = Math.min(fa, fb) + 1; f < Math.max(fa, fb); f++) {
		out.push(topo.at([f, r]))
	}
	return out
}
 ...
 * the king and the rook cross or land on is empty (apart from the king and the rook themselves).
 *
 * @param {object} V variant
 ...
 * @return {object[]}
 */
export function castlingMoves(V, w, side) {
	const out = []
	const topo = V.topology
 ...
 * @param {string} [pawn] the pawn type
 * @return {object[]}
 */
export function pawnExtras(V, w, side, canDouble, pawn = 'p') {
	const out = []
	const topo = V.topology
	const fwd = V.orient(side, [0, 1])
	for (let id = 0; id < w.sq.length; id++) {
		...
		if (w.x.ep >= 0) {
			for (const dx of [1, -1]) {
				const t = topo.step(s, V.orient(side, [dx, 1]))
				const victim = w.board[w.x.epVictim]
				if (t === w.x.ep && w.board[t] === -1 && victim >= 0 && V.enemies(side, w.sd[victim])) {
					out.push({
						key: moveKey(V, s, t),
						from: s,
						to: t,
...
/**
 * Per-world bookkeeping after an orthodox move: the en passant square and the castling rights.
 *
 * @param {object} V variant
...
export function orthodoxAfterMove(V, next, m) {
	if (m.kind === 'double') {
		const [f, r1] = V.topology.coords[m.from]
		const r2 = V.topology.coords[m.to][1]
		next.x.ep = V.topology.at([f, (r1 + r2) / 2])
