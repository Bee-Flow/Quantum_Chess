...
async function dashboard(who) {
	const response = await fetch(
		`${env.baseURL}/ocs/v2.php/apps/dashboard/api/v2/widget-items?widgets[]=quantumchess&format=json`,
		{ headers: authHeaders(who) },
	)
...
test('an invitation is accepted through the notification action', async () => {
	...
	expect(
		upper.opponent.userId,
		'an invitation typed in another letter case stores the canonical user id',
	).toBe(uid('bob'))
	expect(
		(await api('bob', 'GET', 'api/games')).invitations.some((g) => g.id === upper.id),
		'bob sees that invitation',
	).toBe(true)
	await api('admin', 'POST', `api/games/${upper.id}/cancel`)

	game = (await api('admin', 'POST', 'api/games', {
		opponent: uid('bob'),
		rated: true,
		timeControl: 'corr:3d',
	...
	expect(
		game.status === 'pending' && game.ratedRequested && game.colorChoice === 'r',
		'admin invites bob (rated, corr:3d, pending)',
	).toBe(true)
	...
	await expectApiError(
		api('admin', 'POST', 'api/games', { opponent: uid('bob'), rated: true, timeControl: 'corr:none' }),
		400,
		'rated_needs_deadline',
	)
	...
	expect(
		invite.message.includes('3 days per move') && invite.message.includes('Fancy a quantum game?'),
		'invite message: time control and text',
	).toBe(true)
	const acceptAction = invite.actions.find((a) => a.primary)
	expect(
		acceptAction
		&& acceptAction.type === 'POST'
		&& acceptAction.link.includes(`/ocs/v2.php/apps/quantumchess/api/v1/games/${game.id}/accept`),
		'Accept action points to the OCS route',
	).toBe(true)
	...
	expect(
		await findNotification('bob', game.id, 'invited you'),
		'invite notification is gone after accepting',
	).toBeUndefined()
	expect(
		await findNotification('admin', game.id, 'accepted your invitation'),
		'admin is told that bob accepted',
	).toBeTruthy()

	game = (await api('admin', 'GET', `api/games/${game.id}`)).game
	expect(
		game.rated && game.white && game.black && game.chain?.length === 64 && game.deadlineAt > game.now,
		'colours, chain_0, deadline and rated flag set',
	).toBeTruthy()
	expect(
		game.chain,
		'chain_0 equals the JS engine chainStart',
	).toBe(chainStart(game.id, game.white.userId, game.black.userId, game.createdAt))
	...
	expect(
		...
		'joining someone else\'s active game answers like a missing game (no probing)',
...
test('moves: a split, a rolled capture and an idempotent retry', async () => {
	...
	expect(
		r.move.code === 'g1-f3|h3' && r.measurement === null && r.game.ply === 1,
		'white splits the knight g1 → f3|h3',
	).toBe(true)
	expect(
		(await findNotification(black, game.id, 'Your move against'))?.message,
		'black gets your_turn: "They split their knight…"',
	).toContain('split their knight')
	const items = await dashboard(black)
	expect(
		items.items.some((i) => i.link.endsWith(`#/game/${game.id}`)
			&& i.title.startsWith('Your move against')
			&& i.subtitle.startsWith('Move 1')
			&& i.overlayIconUrl.includes('overlay-king-b')),
		'dashboard widget lists the your-move game for black',
	).toBe(true)
	...
	game = r.game
	await expectApiError(
		api(white, 'POST', `api/games/${game.id}/moves`, { code: 'f3-e5', ply: 1, clientId: 'stale-ply-1' }),
		409,
		'conflict',
	)
	...
	expect(
		Number.isInteger(r.measurement?.u) && ['capture', 'miss'].includes(r.measurement.key) && r.replayed === false,
		`rolled capture stored with integer u (${r.measurement?.key}, u=${r.measurement?.u})`,
	).toBe(true)
	const retry = await play(white, game, 'f3-e5', clientId)
	expect(
		retry.replayed === true && retry.measurement.u === r.measurement.u && retry.move.chain === r.move.chain,
		'retry with the same clientId is replayed and never rolls again',
	).toBe(true)
	roll = r
	game = r.game
	...
	expect(
		(await notifications(black))
			.filter((n) => n.object_id === String(game.id) && n.subjectRich.includes('Your move')),
		'one your_turn notification per game',
	).toHaveLength(1)
})
...
test('polling returns only what changed', async () => {
	...
	expect(
		poll.changed === true && poll.moves.length === 1 && poll.moves[0].ply === 2 && poll.game.rev === game.rev,
		'poll with an old rev returns the new move',
	).toBe(true)
...
test('chat: text, quick phrases and mute', async () => {
	...
	expect(
		chat.message.kind === 'text' && chat.message.message === 'Nice   roll!' && chat.rev === game.rev + 1,
		'text message trimmed and control characters removed',
	).toBe(true)
	expect(
		(await findNotification(black, game.id, 'sent a message'))?.message,
		'chat notification with excerpt',
	).toBe('Nice roll!')
	chat = await api(black, 'POST', `api/games/${game.id}/chat`, { phrase: 'good_luck' })
	expect(
		chat.message.kind === 'phrase' && chat.message.message === 'good_luck',
		'quick phrase stored as key',
	).toBe(true)
	await expectApiError(
		api(black, 'POST', `api/games/${game.id}/chat`, { message: 'x'.repeat(501) }),
		400,
		'invalid_argument',
	)
	expect(
		(await api(black, 'PUT', `api/games/${game.id}/mute`, { muted: true })).muted,
		'black mutes the chat',
	).toBe(true)
	await api(white, 'POST', `api/games/${game.id}/chat`, { message: 'muted?' })
	expect(
		(await findNotification(black, game.id, 'sent a message'))?.message,
		'no chat notification while muted',
	).not.toBe('muted?')
...
test('a draw offer is declined through the notification action, with a cool-down', async () => {
	...
	expect(
		await findNotification(black, game.id, 'Your move against'),
		'opening the game clears your_turn',
	).toBeUndefined()
	...
	expect(
		drawNote && drawNote.message === 'Move 2' && drawNote.actions.length === 2,
		'black has the draw_offer notification with Accept/Decline',
	).toBe(true)
	...
	expect(
		game.drawOffer === null && game.canOfferDraw === false && game.drawAvailableAtPly === game.ply + 6,
		'cool-down: next offer after 3 own moves',
	).toBe(true)
	const cooldown = await expectApiError(
		api(white, 'POST', `api/games/${game.id}/draw`, { action: 'offer' }),
		409,
		'draw_not_allowed',
	)
	expect(cooldown.availableAtPly, 'draw_not_allowed carries availableAtPly').toBe(game.ply + 6)
	expect(
		game.chat.map((c) => c.message).join(','),
		'system lines draw_offered and draw_declined',
	).toContain('draw_offered,draw_declined')
...
test('resignation ends the game and updates both ratings', async () => {
	const before = {
		[white]: (await api(white, 'GET', 'api/stats')).online,
		[black]: (await api(black, 'GET', 'api/stats')).online,
	}
	...
	expect(
		...
		&& game.winner === (game.myColor === 'w' ? 'b' : 'w'),
		'white resigns: black wins',
	).toBe(true)
	expect(
		game.ratingChange && game.ratingChange.w + game.ratingChange.b === 0,
		`rating change ${JSON.stringify(game.ratingChange)}`,
	).toBe(true)
	const after = {
		[white]: (await api(white, 'GET', 'api/stats')).online,
		[black]: (await api(black, 'GET', 'api/stats')).online,
	...
	expect(
		after[black].rating > before[black].rating
		&& after[white].rating < before[white].rating
		&& after[black].ratedGames === before[black].ratedGames + 1,
		'both rating rows updated',
	).toBe(true)
	if (before[white].rating === before[black].rating
		&& before[white].ratedGames < 10 && before[black].ratedGames < 10) {
		expect(
			after[black].rating - before[black].rating,
			'K = 40 for provisional players (+20 at equal ratings)',
		).toBe(kw / 2)
	}
	gameOver = await findNotification(black, game.id, 'You won against')
	expect(
		gameOver && gameOver.message.startsWith('Resignation') && gameOver.message.includes('Rating'),
		`winner's game_over: "${gameOver?.message}"`,
	).toBe(true)
...
test('the stored hash chain replays with the JavaScript rules engine', async () => {
	...
	for (const move of full.moves) {
		...
		expect(
			JSON.stringify(applied.measurement),
			`ply ${move.ply}: measurement record replays identically`,
		).toBe(JSON.stringify(move.measurement))
		chain = chainNext(
			chain,
			move.ply,
			move.code,
			move.measurement?.u ?? null,
			move.measurement?.key ?? null,
			applied.state,
		)
		expect(
			chain === move.chain && positionHash(applied.state) === move.stateHash,
			`ply ${move.ply}: chain and state hash match the JS engine`,
		).toBe(true)
		state = applied.state
	}
	expect(
		chain === full.chain && serializeState(state) === serializeState(full.state),
		'head of the chain and final state match',
	).toBe(true)
})
...
test('a rematch through the notification action swaps the colours', async () => {
	...
	expect(
		...
		&& rematch.rematchOf === game.id
		&& rematch.colorChoice === (game.myColor === 'w' ? 'w' : 'b'),
		'OCS rematch creates the swapped-colour invitation',
	).toBe(true)
	expect((await runAction(black, rematchAction)).game.id, 'repeated rematch is idempotent').toBe(rematch.id)
	expect(
		await findNotification(white, rematch.id, 'wants a rematch'),
		'opponent gets the rematch notification',
	).toBeTruthy()
	expect(
		(await dashboard(white)).items[0]?.title,
		'dashboard lists the rematch offer first',
	).toMatch(/wants a rematch$/)
	const started = (await api(white, 'POST', `api/games/${game.id}/rematch`)).game
	expect(
		started.id === rematch.id && started.status === 'active' && started.white.userId === uid(black),
		'the other player\'s rematch accepts it with swapped colours',
	).toBe(true)
	const aborted = await api(
		started.black.userId === uid('admin') ? 'admin' : 'bob',
		'POST',
		`api/games/${started.id}/abort`,
	)
...
test('an invitation accepted on the web, and the abort window', async () => {
	let g2 = (await api('bob', 'POST', 'api/games', {
		opponent: uid('admin'),
		rated: false,
		color: 'w',
	...
	expect(
		g2.status === 'active' && g2.white.userId === uid('bob') && g2.deadlineAt === null && !g2.rated,
		'web accept: bob is White, no deadline',
	).toBe(true)
	...
	expect(
		(await api('bob', 'POST', `api/games/${g2.id}/draw`, { action: 'offer' })).game.drawOffer.by,
		'draw offer by white',
	).toBe('w')
	expect(
		(await ocs('admin', 'POST', `api/v1/games/${g2.id}/draw-accept`)).game.resultReason,
		'draw accepted through the OCS route',
	).toBe('agreement')
})

test('an open challenge joined by two players at once has one winner', async () => {
	const open = (await api('carol', 'POST', 'api/games', {
		opponent: null,
		rated: false,
		timeControl: 'corr:1d',
	})).game
	...
	expect(
		(await api('bob', 'GET', 'api/games')).open.some((g) => g.id === open.id),
		'bob sees it in his lobby',
	).toBe(true)
	await expectApiError(api('carol', 'POST', `api/games/${open.id}/join`), 400, 'own_challenge')
	const joins = await Promise.allSettled([
		api('bob', 'POST', `api/games/${open.id}/join`),
		api('admin', 'POST', `api/games/${open.id}/join`),
	])
	const winners = joins.filter((j) => j.status === 'fulfilled')
	const losers = joins.filter((j) => j.status === 'rejected').map((j) => j.reason)
	expect(
		winners.length === 1 && losers.length === 1 && [409, 404].includes(losers[0].status),
		`one join wins, the other gets ${losers[0]?.status} ${losers[0]?.body?.error}`,
	).toBe(true)
	const joined = winners[0].value.game
	expect(
		await findNotification('carol', open.id, 'joined your open challenge'),
		'carol gets open_joined',
	).toBeTruthy()
	await api(
		joined.white.userId === uid('carol') ? 'carol' : (joined.white.userId === uid('bob') ? 'bob' : 'admin'),
		'POST',
		`api/games/${open.id}/abort`,
	)
})
...
test('the lobby summary, the history and recent opponents', async () => {
	...
	expect(
		typeof summary.rev === 'string' && Number.isInteger(summary.yourTurn) && Number.isInteger(summary.invitations),
	...
	expect(
		(await api('bob', 'GET', 'api/users/recent')).users.some((u) => u.userId === uid('admin')),
		'recent opponents',
	).toBe(true)
})
