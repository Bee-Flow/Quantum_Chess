...
test('the app page carries the initial states, and the share link redirects', async () => {
	...
	expect(page.headers.get('content-security-policy') ?? '', 'CSP allows same-origin workers').toMatch(/worker-src[^;]*'self'/)
	const states = {}
	...
	expect(Object.keys(states), 'initial states').toEqual(expect.arrayContaining(['user', 'features', 'preferences', 'lobby', 'trainerProgress', 'appVersion']))
	expect(states.user.uid === bob.uid && states.features.multiplayer === true && Array.isArray(states.lobby.yourTurn), 'user, features and lobby have the documented shape').toBe(true)
	expect(!Array.isArray(states.preferences) && !Array.isArray(states.trainerProgress), 'preferences and trainer progress are objects').toBe(true)
	const share = await fetch(appUrl('g/42'), { headers: { Authorization }, redirect: 'manual' })
	expect(share.status === 303 && share.headers.get('location').endsWith('/apps/quantumchess/#/game/42'), 'share link redirects to #/game/42').toBe(true)
})

test('statistics, local results and the leaderboard', async () => {
	const stats = await api('bob', 'GET', 'api/stats')
	expect(Number.isInteger(stats.online.rating) && 'askListing' in stats.online && Array.isArray(stats.ratingHistory), `online stats (rating ${stats.online.rating}, ${stats.online.ratedGames} rated games)`).toBe(true)
	expect(stats.local.engine['1'] && stats.local.hotseat && typeof stats.local.llm === 'object' && !Array.isArray(stats.local.llm), 'local stats shape').toBeTruthy()
	...
	expect((await api('bob', 'GET', 'api/stats')).local.llm.professor.d, 'recording an AI opponent draw').toBeGreaterThanOrEqual(1)
	await expectApiError(api('bob', 'POST', 'api/stats/local', { opponent: 'engine', level: 9, result: 'win' }), 400, 'invalid_argument')
	const board = await api('bob', 'GET', 'api/leaderboard')
	expect(['off', 'opt-in', 'opt-out'].includes(board.mode) && Array.isArray(board.entries) && Array.isArray(board.groups), `leaderboard (${board.mode}, ${board.entries.length} entries)`).toBe(true)
...
test('preferences are stored as they are, and trainer progress merges', async () => {
	...
	expect(prefs.preferences.boardTheme === 'quantum' && prefs.preferences.future.x === 1, 'preferences stored verbatim').toBe(true)
	await expectApiError(api('bob', 'PUT', 'api/settings/preferences', { preferences: { blob: 'x'.repeat(17000) } }), 413, 'too_large')
	...
	expect(l01.done === true && l01.stars === 3 && l01.at === 100 && merged.progress.lessons.L02.done === true, 'lessons merge: done OR, stars max, earliest at').toBe(true)
	expect(merged.progress.xp === 50 && merged.progress.streak.last === '2026-09-22' && merged.progress.streak.best === 4, 'xp max, later streak wins with best max').toBe(true)
