...
/**
 * The admin and personal settings pages (docs/GAME-DESIGN.md §8.2–8.3, IMPLEMENTATION-PLAN §5.6): every section
 * renders without console errors, and saved values persist. The AI routes themselves are checked end to end with
 * the fake provider by tests/api/ai.mjs.
 */
...
test.describe('admin settings', () => {
	test('render every section and save a value @phone', async ({ page }, testInfo) => {
		...
		for (const name of ['Online play', 'Ratings and leaderboard', 'Nextcloud AI', 'Organisation AI provider', 'Personal keys and local servers', 'Limits and notices', 'Diagnostics']) {
			await expect(page.getByRole('heading', { name, exact: true })).toBeVisible()
		}
		await testInfo.attach('admin settings', { body: await page.screenshot({ fullPage: true }), contentType: 'image/png' })
		...
		try {
			...
			await expect.poll(async () => (await api('admin', 'GET', 'api/settings/admin')).invite_expiry_days).toBe(next)
		} finally {
...
test.describe('personal settings', () => {
	...
	test('render every section and save a switch @phone', async ({ page }, testInfo) => {
		...
		await testInfo.attach('personal settings', { body: await page.screenshot({ fullPage: true }), contentType: 'image/png' })

		try {
			await page.getByText('Draw offers', { exact: true }).click()
			await expect.poll(async () => (await api('bob', 'GET', 'api/settings/multiplayer')).notifications.drawOffers).toBe(!before.notifications.drawOffers)
		} finally {
			await api('bob', 'PUT', 'api/settings/multiplayer', { notifications: { drawOffers: before.notifications.drawOffers } })
		}
		const width = await page.evaluate(() => document.documentElement.scrollWidth)
		expect(width, 'no horizontal scrolling').toBeLessThanOrEqual(page.viewportSize().width)
	})
