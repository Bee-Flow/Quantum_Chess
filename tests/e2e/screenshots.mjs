 ...
 * OpenAI-compatible server of tests/api. Every image is written to screenshots/, replacing the old one.
 */
...
import { authFile, hasSavedLogin, saveLogin } from './helpers/auth.mjs'
import { appRoot, appUrl, env, getUser } from './helpers/env.mjs'
...
try {
	...
	// 03 and 05 with an AI source for bob: the fake OpenAI-compatible server plays Captain Collapse
	{
		...
		try {
			...
			await page.locator('.qc-new-game__persona', { hasText: 'Captain Collapse' }).click()
