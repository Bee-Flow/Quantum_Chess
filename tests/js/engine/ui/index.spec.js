...
/**
 * The presentation helpers of src/engine/ui: explainOutcome, diffViews, splitTargets, identityColors.
 */
...
describe('explainOutcome', () => {
	it('is not re-exported by the engine index', () => {
...
describe('identityColors', () => {
	it('assigns in order, keeps, and recycles', () => {
		...
		expect(UI.identityColors(states.slice(0, 2))).toEqual({ 7: 1 })
		expect(UI.identityColors(states.slice(0, 4))).toEqual({ 6: 3, 7: 1, 22: 2 })
		expect(UI.identityColors(states.slice(0, 7))).toEqual({ 6: 3, 23: 4 })
		expect(UI.identityColors(states)).toEqual({ 6: 3, 7: 1, 23: 4 })
		expect(UI.identityColors(states[7], { 6: 3, 23: 4 })).toEqual({ 6: 3, 7: 1, 23: 4 })
		expect(UI.identityColors(E.initialState())).toEqual({})
	})
	...
	it('shares the least used colour when all six are taken', () => {
		...
		const colours = UI.identityColors(s)
