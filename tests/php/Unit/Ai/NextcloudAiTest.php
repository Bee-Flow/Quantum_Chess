...
/**
 * Nextcloud AI through TaskProcessing (docs/SPEC.md §10.2): task type choice, input shapes, ownership, states and
 * deletion after reading.
 */
final class NextcloudAiTest extends TestCase {
	/** @var list<Task> */
	...
	public function testPrefersChatWithSystemPrompt(): void {
		...
		$this->assertSame(['status' => 'pending', 'taskId' => 41], $result);
	...
	public function testStatusOwnershipAndDeletion(): void {
		...
		$this->assertSame(['status' => 'pending'], $provider->status('bob', 7));
		...
		$task->setEndedAt(108);
		$this->assertSame(['status' => 'done', 'purpose' => 'coach', 'text' => '**Answer**', 'durationMs' => 8000], $provider->status('bob', 7));
		$this->assertSame([$task], $this->deleted, 'deleted after reading');

		$task->setStatus(Task::STATUS_FAILED);
		$this->assertSame(['status' => 'error', 'error' => 'bad_response'], $provider->status('bob', 7));
