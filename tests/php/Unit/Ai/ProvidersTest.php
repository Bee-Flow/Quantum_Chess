...
use OCA\QuantumChess\Service\Ai\AnthropicProvider;
use OCA\QuantumChess\Service\Ai\OpenAiProvider;
use OCA\QuantumChess\Service\Ai\ProviderException;
...
/**
 * The direct providers against a mocked IClientService (docs/SPEC.md §10.2, §10.8): request bodies and headers, the
 * unsupported-parameter retry, error mapping, and keys never reaching a log call.
 */
final class ProvidersTest extends TestCase {
	...
	public function testOpenAiRequest(): void {
		...
		$this->assertSame(['status' => 'done', 'text' => '{"move":"e7-e5"}'], $result);
		$call = $this->calls[0];
	...
	public function testUnsupportedParameterIsRetriedOnceAndRemembered(): void {
		...
		$this->assertSame('done', $provider->chat(self::MESSAGES, $this->options())['status']);
	...
	public function testAnthropicRequestAndEffortRetry(): void {
		...
		$this->assertSame(['status' => 'done', 'text' => 'Hello there'], $result);
		$first = $this->calls[0];
