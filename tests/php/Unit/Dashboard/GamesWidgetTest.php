...
final class GamesWidgetTest extends TestCase {
	...
	private function widget(): GamesWidget {
		...
		$l->method('n')->willReturnCallback(fn (string $singular, string $plural, int $count) => str_replace('%n', (string)$count, $count === 1 ? $singular : $plural));
		$url = $this->createMock(IURLGenerator::class);
		$url->method('linkToRouteAbsolute')->willReturnCallback(fn (string $route, array $parameters = []) => match ($route) {
			'quantumchess.page.index' => 'https://cloud.example/apps/quantumchess/',
			default => 'https://cloud.example/avatar/' . $parameters['userId'],
		});
		$url->method('imagePath')->willReturnCallback(fn (string $app, string $image) => '/apps/' . $app . '/img/' . $image);
		$url->method('getAbsoluteURL')->willReturnCallback(fn (string $path) => 'https://cloud.example' . $path);
		$users = $this->createMock(IUserManager::class);
		$users->method('getDisplayName')->willReturnCallback(fn (string $uid) => ['alice' => 'Alice', 'bob' => 'Bob'][$uid] ?? null);
	...
	private function items(): array {
		return array_map(fn (WidgetItem $item) => [$item->getTitle(), $item->getSubtitle(), $item->getLink(), $item->getIconUrl(), $item->getOverlayIconUrl()],
			$this->widget()->getItemsV2('bob')->getItems());
	}

	public function testListsInvitationsAndRematchOffers(): void {
		$this->games = [GameBuilder::pending(), GameBuilder::pending(['id' => 8, 'rematchOf' => 3, 'ratedRequested' => 0, 'timeControl' => 'corr:1d'])];
		$this->assertSame([
			['Alice invited you', '3 days per move · Rated', 'https://cloud.example/apps/quantumchess/#/game/7', 'https://cloud.example/avatar/alice', ''],
			['Alice wants a rematch', '1 day per move · Unrated', 'https://cloud.example/apps/quantumchess/#/game/8', 'https://cloud.example/avatar/alice', ''],
		], $this->items());
	}
	...
	public function testShowsTheMoveNumberAndTheTimeLeft(): void {
		$this->games = [
			...
			GameBuilder::active(['id' => 10, 'ply' => 7, 'turn' => 'b', 'deadlineAt' => null, 'timeControl' => 'corr:none']),
		];
		$items = $this->items();
		$this->assertSame(['Move 2 · 3 days left', 'Move 3 · 5 hours left', 'Move 1 · 1 hour left', 'Move 4'], array_column($items, 1));
		$this->assertSame('Your move against Alice', $items[0][0]);
		$this->assertSame('https://cloud.example/apps/quantumchess/img/overlay-king-b.svg', $items[0][4], 'bob plays Black');
	}
	...
	public function testHasAnEmptyMessageAndIsShownToOnlinePlayersOnly(): void {
		$this->assertSame('No games waiting for your move', $this->widget()->getItemsV2('bob')->getEmptyContentMessage());
