...
final class ApiExceptionTest extends TestCase {
	...
	public function testNamedConstructors(): void {
		...
		$this->assertSame(
			['invalid_argument', 400, 'Invalid colour', ['field' => 'color']],
			[$invalid->getErrorCode(), $invalid->getStatus(), $invalid->getMessage(), $invalid->getExtra()],
		);
		$this->assertSame(
			[404, 413],
			[ApiException::notFound('x')->getStatus(), ApiException::tooLarge('x')->getStatus()],
		);
		$this->assertSame(
			['invalid_status', 'game_over'],
			[ApiException::invalidStatus('x')->getErrorCode(), ApiException::gameOver('x')->getErrorCode()],
		);
		$conflict = (new GameConflictException('Changed'))->withExtra(['game' => ['id' => 7]]);
		$this->assertInstanceOf(GameConflictException::class, $conflict);
		$this->assertSame(
			['conflict', 409, ['game' => ['id' => 7]]],
			[$conflict->getErrorCode(), $conflict->getStatus(), $conflict->getExtra()],
		);
		$busy = (new ApiException(ApiError::AiBusy, 'Wait', [], 5))->withExtra(['reason' => 'busy']);
		$this->assertSame(
			[ApiError::AiBusy, 5, ['reason' => 'busy']],
			[$busy->getError(), $busy->getRetryAfter(), $busy->getExtra()],
		);
	}
}
