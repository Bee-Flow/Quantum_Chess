...
final class AiSourceServiceTest extends TestCase {
	...
	public function testSourceAvailability(): void {
		...
		$this->assertSame(
			['nextcloud' => false, 'shared' => false, 'personal' => false, 'any' => false, 'default' => null],
			$summary,
		);
		...
		$this->assertSame(
			[ProviderKind::OpenAi, 'https://api.openai.com/v1', 'gpt-5', 'sk-bob-12345678', false],
			[$resolved->kind, $resolved->baseUrl, $resolved->model, $resolved->apiKey, $resolved->allowLocal],
		);
		...
		try {
			...
			$this->assertSame(
				[403, 'ai_unavailable', 'cap_reached'],
				[$e->getStatus(), $e->getErrorCode(), $e->getExtra()['reason']],
			);
		}
	}
