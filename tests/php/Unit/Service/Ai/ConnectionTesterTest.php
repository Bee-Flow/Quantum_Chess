...
final class ConnectionTesterTest extends TestCase {
	...
	public function testStoredKeysAreOnlySentToTheirAddress(): void {
		$this->settings()->setAdmin(['shared_provider' => [
			'preset' => 'custom',
			'baseUrl' => 'https://ai.example.com/v1',
			'model' => 'm',
		]]);
		$this->aiSettings()->setAdminSecret('shared_api_key', 'sk-org-SECRET-1234');
		$this->aiSettings()->setPersonal('bob', [
			'provider' => ['preset' => 'custom', 'baseUrl' => 'https://ai.example.com/v1', 'model' => 'x'],
			'apiKey' => 'sk-bob-PRIVATE-99',
		]);
		$tester = $this->tester($this->clients());
		$tester->test(
			'admin',
			['scope' => 'shared', 'preset' => 'custom', 'baseUrl' => 'https://attacker.example.com/v1'],
			true,
		);
		$tester->test(
			'admin',
			[
				...
				'apiKey' => 'sk-typed-0000',
			],
			true,
		);
		$tester->test(
			'bob',
			['scope' => 'personal', 'preset' => 'custom', 'baseUrl' => 'https://attacker.example.com/v1'],
			false,
		);
		$result = $tester->test(
			'bob',
			['scope' => 'personal', 'preset' => 'custom', 'baseUrl' => 'https://ai.example.com/v1'],
			false,
		);
		$this->assertSame([
		...
		$this->assertSame(
			['ok' => true, 'code' => null, 'modelCount' => 1, 'models' => [['id' => 'm', 'label' => 'm']]],
			$result,
		);
		$tester->test(
			'admin',
			['scope' => 'shared', 'preset' => 'custom', 'baseUrl' => 'https://ai.example.com/v1'],
			true,
		);
		$this->assertSame(
			['https://ai.example.com/v1/models', 'Bearer sk-org-SECRET-1234'],
			$this->sent[4],
			'the saved address gets the saved key',
		);
	}
	...
	public function testRefusals(): void {
		$tester = $this->tester($this->clients());
		$this->assertSame(
			['ok' => false, 'code' => 'url_not_allowed', 'modelCount' => null, 'models' => null],
			$tester->test(
				'bob',
				['scope' => 'personal', 'preset' => 'custom', 'baseUrl' => 'http://localhost:1234/v1'],
				false,
			),
		);
		foreach ([
