...
final class LlmServiceTest extends TestCase {
	...
	public function testMoveThroughAPersonalProvider(): void {
		...
		$client->method('post')->willReturnCallback(function (string $url, array $options) use (&$posted): IResponse {
			...
			$response->method('getBody')->willReturn(json_encode([
				'choices' => [['message' => ['content' => '{"move":"e2-e4","comment":"Hi","mood":"happy"}']]],
			]));
			return $response;
		});
		$clients = $this->createMock(IClientService::class);
		...
		$this->aiSettings()->setPersonal('bob', [
			'provider' => ['preset' => 'openai', 'model' => 'gpt-5-mini'],
			'apiKey' => 'sk-bob-12345678',
		]);
		$result = $this->llm($clients)->requestMove('bob', [
			'source' => 'personal',
			'model' => 'gpt-5',
			'persona' => 'q7',
			'color' => 'w',
		...
		$this->assertSame(
			['status' => 'done', 'move' => 'e2-e4', 'pick' => null, 'comment' => 'Hi', 'mood' => 'happy'],
			$result,
		);
		$this->assertSame('https://api.openai.com/v1/chat/completions', $posted[0][0]);
		$this->assertSame(
			['gpt-5', 3200, 0.7],
			[$posted[0][1]['model'], $posted[0][1]['max_completion_tokens'], $posted[0][1]['temperature']],
	...
	private function sharedModels(): array {
		$this->settings()->setAdmin([
			'shared_enabled' => true,
			'shared_provider' => ['preset' => 'openai', 'model' => 'gpt-5-mini'],
		]);
