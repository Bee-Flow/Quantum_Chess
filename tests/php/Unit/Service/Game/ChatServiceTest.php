...
final class ChatServiceTest extends TestCase {
	...
	public function testChatValidation(): void {
		...
		$this->assertSame(
			'invalid_argument',
			$this->errorOf(fn () => $this->chat()->send(7, 'alice', str_repeat('x', 501), null)),
		);
		$this->assertSame(
			'invalid_argument',
			$this->errorOf(fn () => $this->chat()->send(7, 'alice', null, 'bad_phrase')),
		);
	...
	public function testWhenTheChatIsOpen(): void {
		...
		$this->assertTrue(ChatService::isOpen(
			GameBuilder::finished(['finishedAt' => self::NOW - ChatService::OPEN_AFTER_END]),
			self::NOW,
		));
		$this->assertFalse(ChatService::isOpen(
			GameBuilder::finished(['finishedAt' => self::NOW - ChatService::OPEN_AFTER_END - 1]),
			self::NOW,
		));
		...
		$this->assertSame(
			'chat_closed',
			$this->errorOf(fn () => $this->chat()->send(7, 'alice', 'hi', null)),
			'no chat before the game starts',
		);
