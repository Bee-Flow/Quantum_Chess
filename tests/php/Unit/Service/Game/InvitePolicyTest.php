...
final class InvitePolicyTest extends TestCase {
	...
	private array $groups = ['alice' => ['staff', 'chess'], 'bob' => ['staff'], 'carol' => ['guests'], 'dave' => ['staff']];
	...
	private array $sharing = ['groupsOnly' => false, 'excluded' => [], 'enumeration' => true, 'limitToGroups' => false, 'fullMatch' => false];
	...
	private function policy(): InvitePolicy {
		...
		$groups->method('getUserGroupIds')->willReturnCallback(fn (IUser $user) => $this->groups[$user->getUID()] ?? []);
		$groups->method('isInGroup')->willReturnCallback(fn (string $uid, string $gid) => in_array($gid, $this->groups[$uid] ?? [], true));
		$apps = $this->createMock(IAppManager::class);
		$apps->method('isEnabledForUser')->willReturnCallback(fn (string $app, IUser $user) => in_array($user->getUID(), $this->appUsers, true));
		...
		$share->method('shareWithGroupMembersOnlyExcludeGroupsList')->willReturnCallback(fn () => $this->sharing['excluded']);
		...
		$settings->method('isMultiplayerEnabledFor')->willReturnCallback(fn (string $uid) => in_array(strtolower($uid), $this->multiplayer, true));
		...
		$games->method('countCreated')->willReturnCallback(fn (string $uid, string $status) => $status === Game::STATUS_OPEN ? $this->counts['open'] : $this->counts['pending']);
	...
	public function testMultiplayerUsers(): void {
		...
		$policy = $this->policy();
		$this->assertSame([true, false, false, false], [$policy->isMultiplayerUser('alice'), $policy->isMultiplayerUser('bob'),
			$policy->isMultiplayerUser('dave'), $policy->isMultiplayerUser('nobody')]);
	}
	...
	public static function invitations(): array {
		return [
			...
			'sharing within groups: no shared group' => [['sharing' => ['groupsOnly' => true]], 'alice', 'carol', false],
			'sharing within groups: shared group excluded' => [['sharing' => ['groupsOnly' => true, 'excluded' => ['staff']]], 'alice', 'bob', false],
			'enumeration limited to groups: shared group' => [['sharing' => ['limitToGroups' => true]], 'alice', 'bob', true],
			'enumeration limited to groups: stranger' => [['sharing' => ['limitToGroups' => true]], 'alice', 'carol', false],
			'enumeration limited to groups: played before' => [['sharing' => ['limitToGroups' => true], 'havePlayed' => true], 'alice', 'carol', true],
			'no enumeration' => [['sharing' => ['enumeration' => false]], 'alice', 'bob', false],
			'no enumeration, full match' => [['sharing' => ['enumeration' => false, 'fullMatch' => true]], 'alice', 'carol', true],
			'no enumeration, played before' => [['sharing' => ['enumeration' => false], 'havePlayed' => true], 'alice', 'carol', true],
		];
	}
	...
	public function testAssertCanInviteReturnsTheCanonicalUserId(): void {
		...
		try {
			...
			$this->assertSame(['user_not_found', 404, 'You can\'t invite this user'], [$e->getErrorCode(), $e->getStatus(), $e->getMessage()]);
		}
	}
	...
	public static function limits(): array {
		return [
			...
			'too many open challenges' => [['open' => 3], null, ['too_many_open', 429, 'You already have 3 open challenges.']],
			'invitation' => [['pending' => 9], 'bob', null],
			'too many invitations' => [['pending' => 10], 'bob', ['too_many_invitations', 429, 'You have too many open invitations.']],
			'already invited' => [['pair' => 1], 'bob', ['too_many_invitations', 429, 'You have too many open invitations.']],
			'pending invitations do not limit open challenges' => [['pending' => 10, 'pair' => 1], null, null],
			'too many running games' => [['active' => 30], 'bob', ['too_many_active', 429, 'You have reached the maximum number of running games.']],
		];
	}
	...
	public function testOpenChallengeVisibility(): void {
		...
		$this->assertSame([true, false, false], [$policy->canSeeOpenChallenge('bob', $open), $policy->canSeeOpenChallenge('alice', $open),
		...
		$this->assertFalse($policy->canSeeOpenChallenge('bob', GameBuilder::open(['creatorUid' => null])), 'the creator was deleted');
		$scoped = GameBuilder::open(['scopeGroup' => 'guests']);
		$this->assertSame([false, true], [$policy->canSeeOpenChallenge('bob', $scoped), $policy->canSeeOpenChallenge('carol', $scoped)],
